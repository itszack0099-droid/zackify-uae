...
function TrackPage() {
  ...
  const lookup = async (e?: React.FormEvent) => {
    ...
    let query = supabase
      .from("orders")
      .select(
        "id,order_number,customer_name,phone,status,total,created_at,updated_at,items,tracking_number,courier_name,estimated_delivery,delivery_date,return_deadline,address,city,emirate",
      )
  ...
  const returnExpired = order
    ? RETURN_ELIGIBLE.includes(order.status) && deadline !== null && !withinWindow
    : false;
  ...
  return (
    <Layout>
      <div className="max-w-3xl mx-auto px-4 md:px-6 py-5 md:py-12">
        <div className="flex items-center gap-3 mb-5 animate-fade-in-up">
          ...
          <div>
            ...
            <p className="text-xs md:text-sm text-muted-foreground">
              {user ? "Your saved order opens instantly" : t("track.subtitle")}
            </p>
          </div>
        </div>
        ...
        <form
          onSubmit={lookup}
          className="glass rounded-3xl p-4 md:p-5 space-y-3 mb-6 animate-fade-in-up gold-border"
        >
          ...
          <button
            disabled={loading}
            className="w-full px-6 py-3.5 rounded-full bg-gradient-gold text-deep-green font-semibold shadow-gold disabled:opacity-60 inline-flex items-center justify-center gap-2"
          >
        ...
        {order && (
          <div
            className={`glass rounded-3xl p-4 md:p-6 animate-fade-in-up space-y-5 transition-shadow gold-border ${livePulse ? "shadow-gold ring-2 ring-gold/40" : ""}`}
          >
            ...
            <div className="flex items-center justify-between flex-wrap gap-3 p-4 rounded-xl bg-gradient-to-br from-gold/10 to-transparent border border-gold/20">
              <div>
                ...
                <div
                  className={`font-display text-xl ${cancelled ? "text-destructive" : "text-gold"}`}
                >
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">
                    ...
                    {canRequestReturn && deadline && (
                      <span className="block mt-1 text-gold">
                        {t("track.eta")}:{" "}
                        {new Date(deadline).toLocaleDateString(locale, { dateStyle: "medium" })}
                      </span>
                    )}
                  </div>
                </div>
                  ...
                  {STEPS.map((s, i) => {
                    ...
                    return (
                      <div key={s} className="text-center">
                        <div
                          className={`w-11 h-11 mx-auto mb-2 rounded-full flex items-center justify-center transition-all ${active ? "bg-gradient-gold text-deep-green shadow-gold" : "bg-secondary text-muted-foreground"}`}
                        >
                          <Icon className="w-5 h-5" />
                        </div>
                        <div
                          className={`text-[10px] uppercase tracking-wider ${active ? "text-gold font-semibold" : "text-muted-foreground"}`}
                        >
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                ...
                {order.estimated_delivery && (
                  <Detail
                    ...
                    label={t("track.eta")}
                    value={new Date(order.estimated_delivery).toLocaleDateString(locale, {
                      dateStyle: "medium",
                    })}
                  />
                )}
            ...
            <Detail
              ...
              value={`${order.address}, ${order.city}, ${order.emirate}`}
            />
            ...
            <div className="border-t border-gold/15 pt-4 space-y-2">
              ...
              {order.items?.map((it, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 rounded-2xl bg-card/40 border border-gold/10 p-3"
                >
                  <div className="w-12 h-12 rounded-xl bg-secondary overflow-hidden shrink-0">
                    {it.image ? (
                      <img src={it.image} alt="" className="w-full h-full object-cover" />
                    ) : (
                      <Package className="w-5 h-5 text-muted-foreground m-3.5" />
                    )}
                  </div>
