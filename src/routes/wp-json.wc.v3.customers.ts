...
export const Route = createFileRoute("/wp-json/wc/v3/customers")({
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        const { mapCustomer } = await import("@/lib/wcMap");
        const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

        const cred = await authenticateRequest(request);
        if (!cred) {
          await logEvent({ kind: "api.auth_fail", path: "/wp-json/wc/v3/customers", status: 401 });
        ...
        if (error) {
          await logEvent({ kind: "api.error", path: "/wp-json/wc/v3/customers", status: 500, message: error.message });
